...
-   Courses take place from 9.15-12 and from 13.15-16 and in room HAL 230
...
-   Create a barplot with the package `ggplot2` that presents the share of social workers in all the observed countries.
...
-   Use **`count(x)`** to count how often each value occurs in your variable. This is the most universal strategy to summarize your data. It is applicable to all data types (remember from [introduction to quantitative social research](https://de.wikipedia.org/wiki/Skalenniveau): nominal, ordinal, numeric).
...
-   Calculate summary statistics for both the measurements before versus after the intervention for vitality (`Vitality`), social support (`Sozial)`, mastery (`Mastery`) and work outlooks (`Perspektive`). Measurements before the intervention end with a 1, measurements after the intervention end with a 2. Bind the data together into one large data.frame. Hint for more efficient code: ask ChatGPT to write the code with the `across` function. This allows you perform the same functions (pre-defined functions such as `mean()`, but also own functions) on multiple variables.
...
focarb%>%mutate(Langzeit=Bezugsdauer>12)%>%
  count(Langzeit)%>%
  mutate(Prop=n/sum(n))
...
table<-focarb%>%
  ...
  summarise(Average_Happiness=mean(Vitality1),
                Minimum_Happiness=min(Vitality1),
                Maximum_Happiness=max(Vitality1),
                SD_Happiness=sd(Vitality1),
                Average_Happiness2=mean(Vitality2),
                Minimum_Happiness2=min(Vitality2),
                Maximum_Happiness2=max(Vitality2),
...
table<-focarb %>%
  ...
  summarise(
    across(
      c(Vitality1, Vitality2, Sozial1, Sozial2, Perspektive1,Perspektive2,Mastery1,Mastery2),
      list(
        Average = mean,
        Minimum = min,
        Maximum = max,
...
## Summarizing data: analysing associations between two variables
