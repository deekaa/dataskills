...
-   The two most frequently used operations with text variables are searching for text patterns (`grepl`) and replacing text-parts (`gsub`).
    ...
    -   A selection of metacharacters:
        ...
        -   See also <https://emailregex.com/> for a detailed regex example.
...
Daten22 <- rbind(Maedels_22,Jungs_22)

Daten22 <-Daten22%>%
  mutate(Geschlecht=ifelse(grepl("Mädels",Statistikfeld, useBytes = TRUE),"Girls","Boys"))%>%
...
-   Kontingenz Tabellen
    ...
    -   `prop.table(table(x))`: relative Haeufigkeit

### Frequencies

### Distributional parameters: mean and variance
