...
-   Du bist eine R-Expert:in, weisst aber auch, dass angehende Sozialarbeiter:in in Sachen Programmierung wenig Wissen haben und das nicht technische Begriffe eine alltagssprachliche Erklaerung benötigen.
...
4.  *"What are the three most pressing challenges or problems you encounter in this area?"*
...
*Using the information I provide and the reference material, propose **three realistic examples for each problem** where data science could address challenges or improve processes. Each example should:*
...
|              |      |            |                                           |
|--------------|------|------------|-------------------------------------------|
| `Maedels_23` | 2023 | Moditraeff | Statistik OJA Angebote Burgdorf 2023.xlsx |

|            |      |             |                                        |
|------------|------|-------------|----------------------------------------|
| `Jungs_22` | 2022 | Gieletraeff | OJAOffice_Statistikdaten_Gieltrff.xlsx |

|  |  |  |  |
|------------------|------------------|------------------|------------------|
| `Jungs_23` | 2023 | Gieletraeff, range="A11:B11",col_names = FALSE | Statistik OJA Angebote Burgdorf 2023.xlsx |

|           |      |                 |                                           |
|-----------------|-----------------|-----------------|---------------------|
| `JuBu_23` | 2023 | JuBU Traeff 5&6 | Statistik OJA Angebote Burgdorf 2023.xlsx |
...
-   Check the number of rows in the total data frame using `nrow(data.frame)`. Filter out individuals that are social workers
...
-   There are two ways to combine data:
    ...
    -   Row-wise linkage using an identifier variable with `merge(dataframe1,dataframe2, identifier variable)`.
        ...
        -   Useful when data are not sorted in the same order and when some rows in data frame 1 are not present in data frame 2.
...
DACH<-rbind(Swiss,Germans,Austria)
...
-   To extract only parts of a date or time, use the **`format()`** function.
    ...
    -   For example, to reduce `"2022-04-01 15:21:03"` to `"2022-04-01"`, you can use:
        ...
        datetime <- as.POSIXct("2022-04-01 15:21:03")
...
-   Create a variable `months_since_jan24` from the variable `'Erfasst am'` that indicates the number of months between the current date and the introduction of the Turnhalle offer. Use the following steps:
...
-   The dependent variable can either be continuous or have two categories coded as 0 and 1. In the latter case, it displays the proportion of the individuals with a 1 by category of the independent variable. Multiply with 100 and you have percentages.
...
-   See below for how to do cross tables (the method taught in quantitative social science introduction).
...
4.  Create a grouped mean table on whether numbers were, on average, higher before versus after January 2024. Was it higher or lower after the "Büro" was shifted to the "Turnhalle"?
...
-   There might be situations where we not only want to know whether there is an association between two variables or a difference between two groups but were we
...
-   Examples:
    ...
    -   We not only want to know whether social workers are happier with their lives compared to health care workers but also whether this is due to their job satisfaction.

    -   We want to know whether participants in "Fokus Arbeit" feel more vital after the intervention than the control group irrespective of eventual differences in vitality before the intervention.
...
-   The command looks the following: `lm(dependent_variable ~ independent_variable + third_variable, name_of_dataframe)`.
    ...
    -   Then you get the most relevant results using `huxreg(model)` from the `huxtable` package.

    -   Relevant for the interpretation are the coefficients (first row of the table) and test of the statistical significance (last row). You will get two types of coefficients:
...
selected_data <- as.data.frame(data) %>% select(isco08,stfmjob,happy,essround)
...
selected_data <- selected_data %>%
  mutate(
    ...
    stfmjob = case_when(
      as.numeric(stfmjob) %in% c(66, 77, 88, 99) ~ NA_real_,
      TRUE ~ as.numeric(stfmjob)
    )
  )
...
model <- lm(stfmjob ~ occupation
            , data = selected_data)
model_controlled <- lm(stfmjob ~ occupation + happy
            , data = selected_data)
...
-   Cross tables are a straightforward method for analyzing associations. They summarize the relationship between two categorical variables by displaying the frequency distribution of variables in a matrix format, making it easy to observe patterns and potential correlations. It is universally applicable to all types of data since all variables can be coded into categorical format.

-   There are two basic rules of cross tables

    -   If you have a hypothesis about the causal relationship between two variables: distribute categories of the [dependent variable](https://en.wikipedia.org/wiki/Dependent_and_independent_variables) into rows and categories of the independent variable into columns.

    -   Calculate column percentages: it allows you to interpret differences in the distribution of the dependent variable between categories of the independent variable.

-   Cross tables can be made with `count(dependent variable, independent variable)`.

-   To add column percentages, you need to load the `tidyr` package and add the following code

    -   `%>%`

        `group_by(independent variable)%>%`

        `mutate(Spaltenprozent=n/sum(n)*100)%>%`

        `select(-n)%>%`

        `pivot_wider(names_from = independent variable,`

        `values_from = Spaltenprozent)`
