...
-   To sum up:
    ...
    -   **In practice,** data science can help you...

        -   Design of services

        -   Understanding of effective support services

    -   **In politics,** data science can help you ...

        -   Proof of the need for services
...
-   readxl: Excel files (.xls and .xlsx) read into R
...
```{r Solution: Rechnen und Vergleichen, echo=FALSE, eval=FALSE}
# 1.
((3+4-5)-9)^2
log(1)
...
richtungswechsel[1:2, c(3, 6)] # liest sich: "erste bis zweite Zeile, dritte und sechste Spalte"
...
```{r Solution: Zugriff, echo=FALSE, , eval=FALSE}
# 1.
richtungswechsel <- read.csv("S:/MA1082973/_FHNW/BA472/data sets/Richtungswechsel/Richtungswechsel_anonymized data set.csv")
...
-   Hadley Wickham has tried to write down some conventions for the clean presentation/storage of data
...
## Exercise: Contingency tables

1.  download (if necessary install beforehand) the package `foreign`. Load the data about the sinking of the Titanic with the following command: `titanic <- read.dta("http://www.stata-press.com/data/kkd/titanic2.dta")` and familiarize yourself a little with the data (e.g. `head()` or `summary()`).
2.  create a crosstab with the variables `class` and `survived`.
3.  calculate median and mean age for survived and deceased.

## Solution: Contingency tables

```{r Solution: Kontingenztabellen, echo=FALSE, eval=FALSE}
### 1.
library(foreign)
...
-   This is not a measure of effect:
    ...
    -   Why does a canton (not) reaching the goal of 40% say much about the effects of its measures?
...
-   Design a study on the effect of a social work offer/intervention

-   What are the outcome variables?

-   Which groups would you compare?

## Analyzing social work experiments in R

### Example

```{r rct, echo=TRUE, eval=FALSE, results='asis'}
...
#There are some missing values, but we are not bothered
...
richtungswechsel <- data.anonym%>%
  mutate(vitality.change=vit2-vit1,#Here we calculate a new column measuring the change in vitality
         ...
         Group=ifelse(INT1==1,"Intervention group","Comparison group")) #Here ge give the variable INT1 a nicer, more telling name and labels
...
-   Download data from Fokus Arbeit

-   Inspect data

-   Remove missing values

-   Calculate new variable measuring the change in vitality before versus after the intervention

-   Did the intervention increase vitality?
