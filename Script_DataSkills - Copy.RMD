...
```{r grafik, fig.margin=TRUE}
# Erstelle zwei Vektoren mit den Jahren und den Schätzungen für Version 1
jahr <- c(2004:2023)
sch <- c(4, 5, 6, 7,8, 7, 8, 8, 7, 7, 8, 7, 8, 4, 3, 3, 2, 2, 2, 2)
# Erstelle eine Tabelle mit den Vektoren als Spalten
tab1 <- data.frame(jahr, sch,typ="Lebenszufriedenheit")
save<-sch
jahr <- c(2004:2023)
sch <- c(4, 5, 6, 7,8, 7, 8, 8, 7, 7, 8, 7, 8, 9, 9, 9, 8,9, 9,9)
# Erstelle eine Tabelle mit den Vektoren als Spalten
tab2 <- data.frame(jahr, sch,typ="Lebenszufriedenheit")
tab3<-data.frame(jahr,sch=save-sch,typ="Wirkung Scheidung",version="Wirkung der Scheidung")
library(ggplot2)
# Füge eine neue Spalte hinzu, die die Version angibt
tab1$version <- "Reale Situation"
tab2$version <- "Kontrafaktische Geschichte"
# Füge die beiden Tabellen zusammen
tab <- rbind(tab1, tab2,tab3)
# Erstelle einen Liniendiagramm mit ggplot ()

plot(ggplot(tab,       aes(x = jahr,           y = sch,           color = version)) +  
geom_line()+  #geom_bar(stat="identity",position="dodge") + 
 facet_grid(typ~.,scales="free_y")+ 
labs(title="Die Wirkung von Evas Schiedung",       subtitle = "Lebenszufriedenheit von Eva in der tatsächlichen und in der kontrafaktischen Welt",       x = "Jahr",color="Wirkung",       y = "Lebenszufriedenheit resp.\nWirkung der Scheidung auf Lebenszufriedenheit")+  
