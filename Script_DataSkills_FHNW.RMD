...
combined_df <- data.frame(
  ...
  percent = c("", round(geschl_tab$Freq/106*100,2), "", round(Age_tab$Freq/106*100,2))
)
...
```{r Solution: Package dplyr, eval=F, echo=TRUE}
### read data 
library(haven)
library(dplyr)
...
```{r Solution: Grafiken mit ggplot2, eval=F}
...
#The advantage with OLS is transparency; we know how individual determinants contribute to the prediction
...
-   Can you predict more accurately when using the random forest algorithm (package: randomForest) than OLS?
...
```{r Prediction_Excercise, eval=FALSE,echo=TRUE}
library(dplyr)
